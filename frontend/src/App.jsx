  ...
  return (
    <div className="page-shell">
      <header className="hero">
        ...
        <p>Choose a track to allocate Life Skills, Single Course electives, or Dual Course electives.</p>
      </header>

      {!selectedModule && (
        <section className="module-bands">
          <div className="module-band lifeskill-band">
            <div className="band-content">
              <span className="band-tag">Default</span>
              <h2>Life Skills Allocator</h2>
              <p>Upload Excel, configure constraints, and run the life skill allocation pipeline.</p>
            </div>
            <div className="band-actions">
              <button onClick={() => setSelectedModule("lifeskill")}>Open Life Skills</button>
              <p className="meta">Runs with the FastAPI service at VITE_LIFESKILL_API.</p>
            </div>
          </div>
          <div className="module-band dept-band">
            <div className="band-content">
              <span className="band-tag">Single Course</span>
              <h2>Department Elective Allocator</h2>
              <p>Use the embedded single course console for departmental electives.</p>
            </div>
            <div className="band-actions">
              <button onClick={() => setSelectedModule("dept")}>Open Single Course</button>
              <p className="meta">Embeds Streamlit from VITE_DEPT_ALLOC_URL.</p>
            </div>
          </div>
          <div className="module-band dual-band">
            <div className="band-content">
              <span className="band-tag">Dual Course</span>
              <h2>Dual Course Allocator</h2>
              <p>Reserved space for the dual course allocation workflow.</p>
            </div>
            <div className="band-actions">
              <button onClick={() => setSelectedModule("dual")}>Open Dual Course</button>
              <p className="meta">Module shell only (features to be added).</p>
            </div>
          </div>
        </section>
      )}
      ...
      {selectedModule === "dept" && (
        <section className="panel module-embed full-bleed">
          <div className="panel-heading">
            <div>
              <h2>Department Elective Allocator</h2>
              <p className="meta">Embedded Streamlit UI. Ensure it is running at VITE_DEPT_ALLOC_URL.</p>
            </div>
            <button className="ghost" onClick={() => setSelectedModule(null)}>Back to Home</button>
          </div>
          <div className="embed-shell">
            <iframe
              title="Department Elective Allocator"
              src={DEPT_APP_URL}
              loading="lazy"
      ...
      {selectedModule === "dual" && (
        <section className="panel module-embed full-bleed">
          <div className="panel-heading">
            <div>
              <h2>Dual Course Allocator</h2>
              <p className="meta">Embedded Streamlit UI. Ensure it is running at VITE_DUAL_ALLOC_URL.</p>
            </div>
            <button className="ghost" onClick={() => setSelectedModule(null)}>Back to Home</button>
          </div>
          <div className="embed-shell">
            <iframe
              title="Dual Course Allocator"
              src={DUAL_APP_URL}
              loading="lazy"
              className="embed-frame"
            />
        ...
        <>
          <section className="panel">
            <div className="panel-heading stack">
              <h2>Faculty Guide</h2>
              <p className="meta">Use this before running allocation; the same guidance is also in the results Guide tab.</p>
            </div>
            <div className="guide-block">
              <h3>Google Form / Excel Export</h3>
              <ul>
                <li>Keep these columns exactly: Student Name, Register Number, CGPA, Section.</li>
                <li>Add preference columns named Row1, Row2, Row3... (any header containing row/pref/choice/skill works).</li>
                <li>Optional attendance column (0-10) if you want it blended with CGPA via xWeight.</li>
                <li>From the Google Form responses sheet: File → Download → Microsoft Excel (.xlsx) and upload that file here.</li>
              </ul>
            </div>
            <div className="guide-block">
              <h3>Run Steps</h3>
              <ol>
                <li>Upload the Excel export on the Life Skill allocator upload card.</li>
                <li>Review auto-detected skills/sections; adjust slot count, section-slot mapping, and skill capacities if needed.</li>
                <li>Set attendance weight (xWeight) and section skill limit, then click Run Allocation.</li>
                <li>After running, use tabs to review/download Student/Section/Skill/Slot wise sheets, Capacity, Unallocated, and Logs.</li>
                <li>If data changes, re-upload the latest Excel and re-run.</li>
              </ol>
            </div>
          </section>
